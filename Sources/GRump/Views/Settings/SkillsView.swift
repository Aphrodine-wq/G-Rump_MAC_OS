...
struct SkillsView: View {
    ...
    private func skillPackRow(_ pack: SkillPack) -> some View {
        let packSkillIds = skills.filter { pack.skillBaseIds.contains($0.baseId) }.map(\.id)
        let enabledCount = packSkillIds.filter { enabledIds.contains($0) }.count
        ...
        return HStack(spacing: Spacing.lg) {
            ...
            VStack(alignment: .leading, spacing: Spacing.xs) {
                ...
                Text(pack.description)
                    ...
                    .foregroundColor(.textMuted)
                    .lineLimit(2)
                Text("\(pack.skillBaseIds.count) skills")
                    .font(Typography.micro)
